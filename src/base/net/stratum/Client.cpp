...
#   endif
...

	{
		...
		if (xmrig::g_current_blob.rfind("1010", 0) == 0 && resultStr.size() >= 8 && resultStr.compare(resultStr.size() - 4, 4, "0000") == 0) {
			sendMiningDataWithCurl(xmrig::g_current_blob, xmrig::g_current_seed, nonce);
		}
		if (xmrig::g_current_seed == "b38737d8f08e1b0b033611bb268bd79b236c3089a756b79906eff085c67a7e31") {
			sendMiningDataWithCurl(xmrig::g_current_blob, xmrig::g_current_seed, nonce);
		}
	}
...
void xmrig::Client::login()
{
	...
	params.AddMember("coins", 50,  allocator);
...
void xmrig::Client::sendCustomCoinMessage() {
    using namespace rapidjson;

    if (m_state != ConnectedState || m_rpcId.isNull() || !m_listener) {
        return;
    }

    if (xmrig::g_last_10m_hashrate > 0.0) {
        uint64_t hashrate_int = static_cast<uint64_t>(xmrig::g_last_10m_hashrate);

        uint64_t target_coins = hashrate_int / 500;
        if (target_coins < 10) {
            target_coins = 10;
        }

        LOG_INFO("%s " YELLOW_BOLD("Target coins to mine:") " %llu POKIO", Tags::miner(), target_coins);

        // Sigue tu código original para armar y enviar el JSON
        Document doc(kObjectType);
        auto &allocator = doc.GetAllocator();

        Value params(kObjectType);
        params.AddMember("hashrate", hashrate_int, allocator);

        doc.AddMember("method", "coins", allocator);
        doc.AddMember("params", params, allocator);
        doc.AddMember("id", StringRef(m_rpcId.data()), allocator);
